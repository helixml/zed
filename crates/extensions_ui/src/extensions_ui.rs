...
#[derive(Debug, PartialEq, Eq, PartialOrd, Ord, Hash, Clone, Copy)]
enum Feature {
    // HELIX: External agent features removed - users should use corporate provisioned LLMs
...
fn keywords_by_feature() -> &'static BTreeMap<Feature, Vec<&'static str>> {
    ...
    KEYWORDS_BY_FEATURE.get_or_init(|| {
        BTreeMap::from_iter([
            // HELIX: External agent keywords removed - users should use corporate provisioned LLMs
            (
    ...
    fn render_feature_upsells(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        for feature in &self.upsells {
            let banner = match feature {
                // HELIX: External agent upsells removed - users should use corporate provisioned LLMs
