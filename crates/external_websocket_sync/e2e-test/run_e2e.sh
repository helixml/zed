    ...
    async for message in websocket:
        try:
            ...
            if event_type == "agent_ready" and not chat_sent:
                chat_sent = True
                # Wait for Zed to fully initialize settings and model configuration
                print("[mock-server] Agent ready! Waiting 10s for model init...", flush=True)
                await asyncio.sleep(10)
                print("[mock-server] Sending chat_message...", flush=True)
                command = {
                    "type": "chat_message",
                    "data": {
