...
impl ConversationView {
    ...
    pub fn from_existing_thread(
        ...
        agent: Rc<dyn AgentServer>,
        connection_store: Entity<super::agent_connection_store::AgentConnectionStore>,
        connection_key: super::Agent,
        workspace: WeakEntity<Workspace>,
        project: Entity<Project>,
        thread_store: Option<Entity<ThreadStore>>,
        prompt_store: Option<Entity<PromptStore>>,
        ...
        let display_name: SharedString = agent_id.0.clone();
            ...
            this.update_in(cx, |this, window, cx| {
                match result {
                    Ok(thread) => {
                        ...
                        {
                            if !is_resume {
                                let (acp_thread_id, title_opt) = this.active_thread()
                                    .map(|tv| {
                                        let thread = &tv.read(cx).thread;
                                        let t = thread.read(cx);
                                        (t.session_id().to_string(), t.title().map(|t| t.to_string()))
                                    })
                                    .unwrap_or_default();
    ...
    fn handle_thread_event(
        ...
        match event {
            AcpThreadEvent::NewEntry => {
                ...
                // WebSocket event forwarding for NewEntry is handled by thread_service.rs
                // (see create_new_thread_sync / handle_follow_up_message / load_thread_from_agent)
            }
            AcpThreadEvent::EntryUpdated(index) => {
                ...
                // WebSocket event forwarding for EntryUpdated is handled by thread_service.rs
            }
            ...
            AcpThreadEvent::Stopped(stop_reason) => {
                ...
                // History refresh handled by ThreadView subscription
            }
            AcpThreadEvent::Refusal => {
            ...
            AcpThreadEvent::TitleUpdated => {
                ...
                #[cfg(feature = "external_websocket_sync")]
                {
                    ...
                    let title_str = thread.read(cx).title().map(|t| t.to_string()).unwrap_or_default();
