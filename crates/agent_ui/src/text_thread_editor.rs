...
impl TextThreadEditor {
    ...
    fn is_websocket_triggered(&self, cx: &Context<Self>) -> bool {
        ...
        let messages: Vec<_> = text_thread.messages(cx).collect();
    ...
    fn handle_text_thread_event(
        ...
        match event {
            ...
            TextThreadEvent::Operation(op) => {
                // Check for message completion to send message_completed event
                #[cfg(feature = "external_websocket_sync")]
                {
                    use assistant_text_thread::{TextThreadOperation, MessageStatus};

                    if let TextThreadOperation::UpdateMessage { message_id, metadata, .. } = op {
                        ...
                        if metadata.status == MessageStatus::Done {
                            ...
                            // Get text thread ID and notify via external websocket sync
                            let text_thread_id = self.text_thread.read(cx).id();
                            if let Some(sync) = external_websocket_sync::ExternalWebSocketSync::global(cx) {
                                sync.notify_message_completed(&text_thread_id, message_id);
