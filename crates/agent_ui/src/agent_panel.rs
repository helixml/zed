...
impl AgentPanel {
    ...
    fn process_websocket_thread_requests_deferred(&mut self, cx: &mut Context<Self>) {
        ...
        for request in pending_requests {
            ...
            if let Some(context_id) = existing_context_id {
                log::error!("🔄 [AGENT_PANEL] Thread already exists for session {}, context_id: {}", 
                           request.external_session_id, context_id.to_proto());
                // TODO: Switch to existing thread when we have window access
            } else {
                log::error!("🆕 [AGENT_PANEL] Creating NEW thread for session: {}", request.external_session_id);
                
                // Create a new context directly through the context store
                let context = self.context_store.update(cx, |context_store, cx| {
                    context_store.create(cx)
                });
                ...
                // Add the initial message to the context
                context.update(cx, |context, cx| {
                    ...
                    log::error!("🤖 [AGENT_PANEL] Triggering AI assistant for message: {}", request.message);
                    context.assist(cx);
                });
            }
        }
    ...
    fn new_acp_thread_with_message(
        ...
        // Create the ACP thread view synchronously with window access
        let thread_view = cx.new(|cx| {
            crate::acp::AcpThreadView::new(
                server.clone(),
                None, // resume_thread
                                    ...
                                    if let Some(sender) = cx.try_global::<external_websocket_sync::WebSocketSender>() {
                                        ...
                                        if let Some(ws_sender) = sender_guard.as_ref() {
                                            ...
                                            if let Ok(json_str) = serde_json::to_string(&message) {
                                                ...
                                                if let Err(e) = ws_sender.send(Message::Text(json_str.into())) {
                ...
                if !initial_message.is_empty() {
                    ...
                    let _ = thread_view.update(cx, |view, cx| {
                        if let Some(thread) = view.thread() {
                            ...
                            log::error!("⚠️ [ACP_CREATE] Thread not ready to receive message");
                        }
                    });
                }
            }
...
mod external_agent_tests {
    use super::*;
    use acp_thread::{AcpThread, StubAgentConnection};
    ...
    ) -> (Entity<AgentPanel>, Entity<Workspace>, VisualTestContext) {
    ...
    async fn test_new_acp_thread_with_message_creates_thread(cx: &mut TestAppContext) {
        init_test(cx);
        let (_panel, _workspace, mut cx) = setup_agent_panel(cx).await;

        // Verify the test setup works and we can create the panel
        cx.run_until_parked();
    }

    #[gpui::test]
    async fn test_session_mapping_stored_correctly(cx: &mut TestAppContext) {
        init_test(cx);
        let (panel, _workspace, mut cx) = setup_agent_panel(cx).await;

        let helix_session_id = "test-helix-session-123".to_string();
        let test_message = "Hello from test";

        // Trigger ACP thread creation
        panel.update_in(&mut cx, |panel, window, cx| {
            panel.new_acp_thread_with_message(test_message, helix_session_id.clone(), window, cx);
        });

        cx.run_until_parked();

        // Verify session mapping was created
        let mapping_exists = cx.read(|_window, cx| {
        ...
        assert!(
            mapping_exists,
            "Session mapping should exist for Helix session ID"
        );
    }

    #[gpui::test]
    async fn test_reverse_session_mapping_stored(cx: &mut TestAppContext) {
        init_test(cx);
        let (panel, _workspace, mut cx) = setup_agent_panel(cx).await;

        let helix_session_id = "test-helix-reverse-456".to_string();
        let test_message = "Test reverse mapping";
        ...
        // Verify reverse mapping was created (ACP session ID -> Helix session ID)
        let reverse_mapping_exists = cx.read(|_window, cx| {
        ...
        assert!(
            ...
            "Reverse session mapping should be created"
        );
    }

    #[gpui::test]
    async fn test_active_view_set_to_external_agent_thread(cx: &mut TestAppContext) {
        init_test(cx);
        let (panel, _workspace, mut cx) = setup_agent_panel(cx).await;

        let helix_session_id = "test-view-789".to_string();
        let test_message = "Test view switch";

        panel.update_in(&mut cx, |panel, window, cx| {
            panel.new_acp_thread_with_message(test_message, helix_session_id, window, cx);
        });

        cx.run_until_parked();

        // Verify the active view was set to ExternalAgentThread
        ...
        assert!(
            is_external_thread,
            "Active view should be set to ExternalAgentThread"
        );
    }

    #[gpui::test]
    async fn test_message_sent_to_acp_thread(cx: &mut TestAppContext) {
        init_test(cx);
        let (panel, _workspace, mut cx) = setup_agent_panel(cx).await;

        let helix_session_id = "test-message-send-101".to_string();
        let test_message = "This message should be sent to ACP thread";

        panel.update_in(&mut cx, |panel, window, cx| {
            panel.new_acp_thread_with_message(
                test_message,
                helix_session_id.clone(),
                window,
                cx,
            );
        });
        ...
        // Verify thread was created and is accessible
        let thread_exists = panel.read_with(&cx, |panel, _cx| {
            if let ActiveView::ExternalAgentThread { thread_view } = &panel.active_view {
                thread_view.is_upgradable()
            } else {
                false
            }
        });

        assert!(thread_exists, "ACP thread view should exist");
    }

    #[gpui::test]
    async fn test_selected_agent_set_to_native_agent(cx: &mut TestAppContext) {
        init_test(cx);
        let (panel, _workspace, mut cx) = setup_agent_panel(cx).await;

        // Set to different agent first
        panel.update(&mut cx, |panel, cx| {
            panel.selected_agent = AgentType::Cloud;
            cx.notify();
        });

        let helix_session_id = "test-agent-type-202".to_string();
        let test_message = "Test agent type";
        ...
        // Verify selected agent was set to NativeAgent
        let is_native_agent = panel.read_with(&cx, |panel, _cx| {
            panel.selected_agent == AgentType::NativeAgent
        });

        assert!(
            is_native_agent,
            "Selected agent should be set to NativeAgent"
        );
    }

    #[gpui::test]
    async fn test_thread_creation_with_empty_message(cx: &mut TestAppContext) {
        init_test(cx);
        let (panel, _workspace, mut cx) = setup_agent_panel(cx).await;

        let helix_session_id = "test-empty-303".to_string();
        let empty_message = "";

        panel.update_in(&mut cx, |panel, window, cx| {
            panel.new_acp_thread_with_message(empty_message, helix_session_id.clone(), window, cx);
        });

        cx.run_until_parked();
        cx.executor().run_until_parked();

        // Thread should still be created even with empty message
        let thread_exists = panel.read_with(&cx, |panel, _cx| {
            matches!(panel.active_view, ActiveView::ExternalAgentThread { .. })
        });
        ...
        assert!(
            thread_exists,
            "ACP thread should be created even with empty message"
        );
    }

    #[gpui::test]
    async fn test_multiple_threads_have_distinct_sessions(cx: &mut TestAppContext) {
        init_test(cx);
        let (panel, _workspace, mut cx) = setup_agent_panel(cx).await;

        let helix_session_1 = "test-multi-1-404".to_string();
        let helix_session_2 = "test-multi-2-505".to_string();

        // Create first thread
        panel.update_in(&mut cx, |panel, window, cx| {
            panel.new_acp_thread_with_message("First message", helix_session_1.clone(), window, cx);
        });
        ...
        // Verify both sessions exist in mapping
        let both_sessions_mapped = cx.read(|_window, cx| {
            if let Some(session_mapping) = cx.try_global::<ExternalSessionMapping>() {
                let sessions = session_mapping.sessions.read();
                sessions.contains_key(&helix_session_1) && sessions.contains_key(&helix_session_2)
            } else {
                false
            }
        });
        ...
        assert!(
            both_sessions_mapped,
            "Both Helix sessions should be mapped to distinct ACP sessions"
        );
    }
}
