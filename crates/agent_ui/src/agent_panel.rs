...
impl AgentPanel {
    ...
    pub fn load(
        ...
        cx.spawn(async move |cx| {
            ...
            // Helix: wait for WebSocket to connect before restoring threads so the
            // agent_ready → open_thread handshake can complete. The WS connection
            // is local or DC-to-DC so this is fast (~100ms). See PR helixml/zed#X.
            #[cfg(feature = "external_websocket_sync")]
            {
            ...
            let panel = workspace.update_in(cx, |workspace, window, cx| {
                ...
                panel.update(cx, |panel, cx| {
                    ...
                    if let Some(metadata) = terminal_to_restore {
                        ...
                        // Helix: nothing to restore — send agent_ready immediately so the
                        // server knows Zed is ready and can flush its queue. Without this,
                        // the server waits 60s for agent_ready. When a thread IS restored,
                        // agent_ready is sent by the initial_state completion handler in
                    ...
                    this.update_in(cx, |this, window, cx| {
                        ...
                        if let BaseView::AgentThread { conversation_view } = &this.base_view {
                            if let Some(active_thread) = conversation_view.read(cx).active_thread() {
                                ...
                                eprintln!("⚠️ [AGENT_PANEL] No active_thread found after set_base_view, cannot activate follow");
                            }
                        }
    ...
    pub fn load_agent_thread(
        ...
        // Helix-mode entity-identity guard (Critical Fix #11). The thread_id-based
        // dedup below can miss when the active CV's underlying `Entity<AcpThread>`
        // has been replaced (e.g. by a `notify_thread_display` callback wrapping a
        // live registered entity) but the CV still reports the same thread_id. The
        // bug this prevents: clicking the currently-open thread caused
        // `external_thread` to issue a duplicate `connection.load_session()`,
        // splitting the panel from the entity Helix is subscribed to.
        //
        // Look up the session_id for this thread first (drafts may not have one
        // yet; fall through to upstream's logic in that case).
        #[cfg(feature = "external_websocket_sync")]
        if let Some(session_id) = ThreadMetadataStore::try_global(cx).and_then(|store| {
            store
                .read(cx)
                .entry(thread_id)
                ...
                .and_then(|w| w.upgrade())
            {
                ...
                let retained_key = self
                    ...
                    .map(|(id, _)| *id);
                if let Some(retained_thread_id) = retained_key {
                    if let Some(conversation_view) =
                        self.retained_threads.remove(&retained_thread_id)
                    {
                        self.set_base_view(
                            BaseView::AgentThread { conversation_view },
                            focus,
...
impl Panel for AgentPanel {
    ...
    // HELIX: honour the `auto_open_panel` agent setting (porting guide item #26).
...
impl AgentPanel {
    fn ensure_thread_initialized(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if matches!(self.base_view, BaseView::Uninitialized) {
            // Helix Fix 1b: under external_websocket_sync, suppress speculative
            // initialization of any thread/terminal here. Auto-init fires every
            // time the panel becomes active (incl. workspace restore on each
            // container restart), and any path that calls connection.new_session()
            // spawns a Claude ACP child + its full MCP server tree that contends
            // with the user's real Claude for the npm `_npx/<hash>` cache,
            // producing 180s `chrome-devtools/github context server failed to
            // start` timeouts. Helix drives all real conversations through the
            // chat_message WebSocket path (create_new_thread_sync), so leaving
            // the panel Uninitialized here is functionally fine for spec-task
            // agents. THIS MUST REMAIN THE FIRST STATEMENT INSIDE THE
            // BaseView::Uninitialized BRANCH — upstream PR #57150 added the
            // terminal-spawn branches below, which would also spawn agent
            // processes if executed under Helix sync. Phase 17 of the E2E is
            // the regression gate. See:
