...
- **E2E `claude` round — Phase 17 (queue interrupt) does not complete locally**: the interrupt correctly cancels the running turn (`X → interrupted` ✅) but the follow-up interrupt message `Y` is not redelivered within the 60 s window. Reproduced twice. **This is NOT a regression from this merge**: (a) the `zed-agent` round passes Phase 17 (the merged interrupt/cancel Rust path is correct); (b) Phase 17 is brand-new from PR #66 (task 002228) whose own `pull_request_zed.md` states it "Runs green locally (`run_docker_e2e.sh`, **zed-agent**)" — it was **never validated for the `claude` agent**, even before this merge; (c) the failure is in `claude-agent-acp` subprocess post-cancel redelivery (the fragile area PR #63 wedge-recovery exists for), amplified by the model substitution. The authoritative `claude`-round gate is CI (real Anthropic model + secrets). Claude phases 1–16 pass.
