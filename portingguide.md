...
- **Build**: `./stack build-zed dev --features external_websocket_sync` — **green** (after the 6 repairs below). Fresh 580 MB binary produced and runs.
- **E2E (Docker), `zed-agent` round**: **ALL 17 phases + store validations PASSED** ("E2E TEST PASSED"), run twice (standalone and as round 1 of the both-agents run). This exercises the full Helix sync surface, the ACP 1.0 fixes, PR #65's `chat_response_error` path (observed firing correctly), the cancel/Stopped invariants (Fix #6/#8/#9), streaming (Phase 15), draft-suppression (Phase 16), and the queue interrupt path (Phase 17).
- **E2E model substitution (local only)**: the local Anthropic proxy (GCP Vertex-backed) returns **404 for `claude-sonnet-5`** (the intended `claude-sonnet-4-5-latest`), so the run used `claude-opus-4-8` via a **temporary, un-committed** edit to `run_e2e.sh` (reverted after the run, per the CLAUDE.md guidance — CI has the real Anthropic secret/model). Not a merge issue.
- **E2E `claude` round — all 17 phases PASS after the interrupt-ordering fix below.** Initially Phase 17 (queue interrupt) failed for `claude` only: the interrupt cancelled the running turn (`X → interrupted` ✅) but the follow-up message `Y` was never delivered (60 s timeout, reproduced 2×). Root-caused to a **pre-existing interrupt-ordering race** (not introduced by this merge — `websocket_sync.rs` interrupt handling and the `thread_service.rs` request_id fallback are byte-for-byte unchanged by the merge; PR #66 only ever validated Phase 17 for `zed-agent`). Fixed in commit `ad863cb42b` (see "Interrupt-ordering fix" below). After the fix: `claude` all 17 phases + store validations **PASSED**; `zed-agent` re-verified all 17 phases **PASSED** (no regression to the shared interrupt path — Phases 8/9/13/17). Back-to-back both-agents runs can still hit the documented `claude` Phase-1 npm-bootstrap flake (retry-permitted).

### Interrupt-ordering fix (claude Phase 17) — commit `ad863cb42b`
**Symptom**: `interrupt=true` follow-up (`Y`) killed instead of the turn it was meant to replace, for the slow `claude-agent-acp` subprocess.
**Root cause**: on `interrupt=true`, `websocket_sync.rs` fired `request_cancel_thread` on the **independent** by-thread cancel task while `request_thread_creation(Y)` ran on the **sequential** creation loop. The two raced: for a slow agent the blanket cancel landed on `Y`'s freshly-started turn, and the cancelled turn's `Stopped` fell back to the global request_id (now `Y`'s), marking `Y` `interrupted` so it never completed. (`zed-agent`'s in-process stub is synchronous, so the race never opened — which is why it always passed.) Helix's `processInterruptPrompt` already sends a **targeted, awaited** `cancel_current_turn` before `Y`, so the blanket by-thread cancel was a redundant duplicate.
**Fix**: thread an `interrupt` flag through `ThreadCreationRequest` instead of the racy independent cancel. The creation handler now cancels the pre-existing running turn **inline, in order, immediately before dispatching** `Y` (`thread_service.rs`). Because the global request_id is still the old turn's at that point (it's rotated to `Y` inside `handle_follow_up_message`, which runs after), the cancelled turn's `Stopped` is attributed correctly, and `Y`'s own turn is never the cancel target. No-op when the turn is already gone (e.g. after the server's targeted cancel). The by-thread `request_cancel_thread` path is left in place (unused by the interrupt flow now) to avoid churn.
